...
//        drawAxesLine(canvas, paintAxes);
    ...
    private void drawCoordinate(Canvas canvas, Paint paint) {
        ...
        for (int i = 0; i <= (yLabel.length - 1); i++) {
            ...
            canvas.drawText("", margin / 4 + offsetX, startY + offsetY, paint);
        }
    }
...
//        }
        canvas.drawText(data[0] / 20 + "", xPoint + 1 * xScale, toY(data[0]) - 5, paintValue);
        canvas.drawText(data[6] / 20 + "", xPoint + 7 * xScale, toY(data[6]) - 5, paintValue);
        canvas.drawText(data[11] / 20 + "", xPoint + 12 * xScale, toY(data[11]) - 5, paintValue);
        canvas.drawText(data[17] / 20 + "", xPoint + 18 * xScale, toY(data[17]) - 5, paintValue);
        canvas.drawText(data[21] / 20 + "", xPoint + 22 * xScale, toY(data[21]) - 5, paintValue);
    }
    ...
    private void drawValues(Canvas canvas, Paint paint, List<int[]> dataList, int color) {
        ...
        for (int i = 1; i <= (xLabel.length - 1); i++) {
            ...
            canvas.drawText(dataList.get(0)[i - 1] + "", startX - 18,
